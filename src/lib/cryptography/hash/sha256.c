...
inline
uint32_t
blrt_SHA256_Maj( uint32_t x, uint32_t y, uint32_t z )
{
    return (x & y) ^ (x & z) ^ (y & z);
}
...
inline
uint32_t
blrt_SHA256_Sig0( uint32_t x )
{
    return blrt_RotateRight32(x, 2) ^ blrt_RotateRight32(x, 13) ^ blrt_RotateRight32(x, 22);
}

static
inline
uint32_t
blrt_SHA256_Sig1( uint32_t x )
{
    return blrt_RotateRight32(x, 6) ^ blrt_RotateRight32(x, 11) ^ blrt_RotateRight32(x, 25);
}
...
blrt_SHA256_sig0( uint32_t x )
{
    return blrt_RotateRight32(x, 7) ^ blrt_RotateRight32(x, 18) ^ (x >> 3);
}
...
blrt_SHA256_sig1( uint32_t x )
{
    return blrt_RotateRight32(x, 17) ^ blrt_RotateRight32(x, 19) ^ (x >> 10);
}
...
blrt_SHA256_MainCycle( uint32_t hash_value[8], const uint32_t data_block[16] )
{
    ...
    for (int k = 0; k < 64; k++)
    {
        t1 = h + blrt_SHA256_Sig1(e) + blrt_SHA256_Ch(e, f, g) + BLRT_SHA256_K[k] + window[k];
        t2 = blrt_SHA256_Sig0(a) + blrt_SHA256_Maj(a, b, c);
        h = g;
        g = f;
        f = e;
        e = d + t1;
